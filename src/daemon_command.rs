...
pub struct CloudDaemonCommand {
    command: ComponentCommand,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CloudDaemonConfigurationFile {
    path: PathBuf,
}

impl CloudDaemonCommand {
    ...
    pub fn configuration(&self) -> Result<DaemonConfiguration> {
        match self.command.signal_file_argument()? {
            ComponentArgument::SignalFile(file) => {
                CloudDaemonConfigurationFile::from_signal_file(file).configuration()
            }
...
impl CloudDaemonConfigurationFile {
